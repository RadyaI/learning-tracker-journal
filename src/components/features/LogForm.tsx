...
const MemoizedTextarea = memo(({ value, onChange, borderColor, placeholder }: any) => {
  return (
    <div className="relative group">
      <textarea
        ...
        className={`w-full min-h-[150px] resize-y rounded-xl border bg-zinc-950 p-4 text-sm leading-relaxed text-white placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-offset-0 transition-colors scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent ${borderColor}`}
      />
    </div>
  );
});
...
export default function LogForm({ editingLog, onCancel }: { editingLog: LogEntry | null, onCancel: () => void }) {
  ...
  const currentStatus = useMemo(() => getCategoryStatus(seconds), [seconds]);
  ...
  const getBorderColor = () => {
    if (editingLog) return 'border-yellow-500/50 focus:border-yellow-500 focus:ring-yellow-500';
    if (isActive && !isPaused) return 'border-emerald-500/50 focus:border-emerald-500 focus:ring-emerald-500';
    return 'border-zinc-700 focus:border-indigo-500 focus:ring-indigo-500';
  };
